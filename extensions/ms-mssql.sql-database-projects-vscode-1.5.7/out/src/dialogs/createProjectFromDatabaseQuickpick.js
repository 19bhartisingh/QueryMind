...
exports.createNewProjectFromDatabaseWithQuickpick = createNewProjectFromDatabaseWithQuickpick;
const vscode = require("vscode");
const path = require("path");
const constants = require("../common/constants");
const utils_1 = require("../common/utils");
const newProjectTool_1 = require("../tools/newProjectTool");
 ...
 */
async function createNewProjectFromDatabaseWithQuickpick(connectionInfo, createProjectFromDatabaseCallback) {
    ...
    while (!dbs) {
        ...
        if (isValidProfile) {
            try {
                ...
                dbs = (await vscodeMssqlApi.listDatabases(connectionUri)).filter((db) => !constants.systemDbs.includes(db)); // Filter out system dbs
            }
            catch (err) {
    ...
    const projectName = await vscode.window.showInputBox({
        ...
        value: (0, newProjectTool_1.defaultProjectNameFromDb)((0, utils_1.sanitizeStringForFilename)(selectedDatabase)),
        validateInput: (value) => {
            return (0, utils_1.isValidBasenameErrorMessage)(value);
        },
        ignoreFocusOut: true,
    });
    if (!projectName) {
    ...
    const defaultProjectSaveLoc = (0, newProjectTool_1.defaultProjectSaveLocation)();
    ...
    while (true) {
        const browseProjectLocation = await vscode.window.showQuickPick(browseProjectLocationOptions, { title: browseProjectLocationTitle, ignoreFocusOut: true });
        ...
        const locationExists = await (0, utils_1.exists)(path.join(projectLocation, projectName));
        if (!locationExists) {
    ...
    const includePermissionsResult = await vscode.window.showQuickPick([constants.noStringDefault, constants.yesString], { title: constants.includePermissionsInProject, ignoreFocusOut: true });
    ...
    let sdkStyle = await (0, quickpickHelper_1.getSDKStyleProjectInfo)();
    if (sdkStyle === undefined) {
        // User cancelled
        return;
    }
    // 8. Configure Sql project default build or not
    const configureDefaultBuild = await vscode.window.showQuickPick([constants.yesString, constants.noString], { title: constants.confirmCreateProjectWithBuildTaskDialogName, ignoreFocusOut: false });
    ...
    const model = {
        ...
        version: "1.0.0.0",
        extractTarget: (0, utils_2.mapExtractTargetEnum)(folderStructure),
        sdkStyle: sdkStyle,
...
//# sourceMappingURL=createProjectFromDatabaseQuickpick.js.map