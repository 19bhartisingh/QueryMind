...
exports.SqlDatabaseProjectTaskProvider = void 0;
 ...
 */
class SqlDatabaseProjectTaskProvider {
    watchers = [];
    sqlTasks = undefined;
    _onDidChangeTasks = new vscode.EventEmitter();
    workspaceFoldersListener;
    buildHelper;
    /**
     * Event that fires when the tasks change (e.g., when .sqlproj files are created, modified, or deleted)
     */
    onDidChangeTasks = this._onDidChangeTasks.event;
    /**
    ...
    resolveTask(task) {
        if (task.definition.type === constants.sqlProjTaskType) {
            ...
            if (typeof definition.fileDisplayName === "string") {
    ...
    async createTasks() {
        ...
        for (const workspaceFolder of workspaceFolders) {
            ...
            for (const uri of sqlProjPaths) {
                ...
                const project = await project_1.Project.openProject(uri.fsPath);
     ...
     */
    getTask(definition, runCodeAnalysis = false) {
        ...
        const buildArgs = this.buildHelper.constructBuildArguments(this.buildHelper.extensionBuildDirPath, definition.projectStyle);
        ...
        const args = [constants.build, definition.filePath, ...buildArgs];
        if (runCodeAnalysis) {
        ...
        const task = new vscode.Task(definition, definition.workspaceFolder ?? vscode.TaskScope.Workspace, taskName, constants.sqlProjTaskType, processExec, constants.problemMatcher);
        task.group = vscode.TaskGroup.Build;
        return task;
    }
}
exports.SqlDatabaseProjectTaskProvider = SqlDatabaseProjectTaskProvider;
//# sourceMappingURL=sqlDatabaseProjectTaskProvider.js.map