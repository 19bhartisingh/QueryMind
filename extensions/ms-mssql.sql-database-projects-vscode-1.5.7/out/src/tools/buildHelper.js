...
class BuildHelper {
     ...
     * @param outputChannel
     */
    async createBuildDirFolder(outputChannel) {
        if (this.initialized) {
            return true;
        }
        if (!(await utils.exists(this.extensionBuildDir))) {
            await fs_1.promises.mkdir(this.extensionBuildDir);
        }
    ...
    async ensureDacFxDllsPresence(outputChannel) {
        ...
        const microsoftBuildSqlVersionConfig = vscode.workspace.getConfiguration(netcoreTool_1.DBProjectConfigurationKey)[constants.microsoftBuildSqlVersionKey];
    ...
    async ensureScriptDomDllPresence(outputChannel) {
     ...
     */
    async ensureNugetAndFilesPresence(nugetName, nugetVersion, expectedFiles, nugetFolderWithExpectedfiles, outputChannel) {
        ...
        for (const fileName of expectedFiles) {
            if (await utils.exists(path.join(buildfilesPath, fileName))) {
                await fs_1.promises.copyFile(path.join(buildfilesPath, fileName), path.join(this.extensionBuildDir, fileName));
            }
        }
        // cleanup extracted folder
        await fs_1.promises.rm(extractedFolderPath, { recursive: true });
        return true;
    }
    /**
     ...
     * @param outputChannel
     */
    async downloadAndExtractNuget(downloadUrl, nugetPath, extractFolderPath, outputChannel) {
        try {
     ...
     */
    constructBuildArguments(buildDirPath, sqlProjStyle) {
        ...
        const isSdkStyle = sqlProjStyle === 0 /* vscodeMssql.ProjectType.SdkStyle */;
        if (!isSdkStyle) {
