...
class NetCoreTool extends shellExecutionHelper_1.ShellExecutionHelper {
    ...
    get isNetCoreInstallationPresent() {
        const netCoreInstallationPresent = !(0, util_1.isNullOrUndefined)(this.netcoreInstallLocation) &&
    ...
    get netcoreInstallLocation() {
        return (vscode.workspace.getConfiguration(exports.DBProjectConfigurationKey)[exports.DotnetInstallLocationKey] || this.defaultLocalInstallLocationByDistribution);
    }
    get defaultLocalInstallLocationByDistribution() {
        switch (this.osPlatform) {
            case exports.winPlatform:
                return this.defaultWindowsLocation;
            case exports.macPlatform:
                return this.defaultMacLocation;
            case exports.linuxPlatform:
            ...
            await new Promise((resolve, reject) => {
                child.on("exit", () => {
                    this.netCoreSdkInstalledVersion = Buffer.concat(stdoutBuffers)
                        ...
                        .trim();
                    try {
                        if (semver.gte(this.netCoreSdkInstalledVersion, exports.minSupportedNetCoreVersionForBuild)) {
    ...
    async runDotnetCommand(options, skipVersionSupportedCheck = false) {
        ...
        try {
            ...
            this._outputChannel.append(vscode_1.l10n.t("\t>>> {0}   … errored out: {1}", command, utils.getErrorMessage(error))); //errors are localized in our code where emitted, other errors are pass through from external components that are not easily localized
            throw error;
        }
    }
    /**
