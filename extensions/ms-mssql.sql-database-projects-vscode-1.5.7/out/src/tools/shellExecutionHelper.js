    ...
    async runStreamedCommand(command, options, sensitiveData = [], timeout = 5 * 60 * 1000) {
        ...
        try {
            ...
            void child.on("exit", (code, signal) => {
                if (code !== null) {
                    this._outputChannel.appendLine(vscode_1.l10n.t("    >>> {0}    … exited with code: {1}", cmdOutputMessage, code));
                }
                else {
                    this._outputChannel.appendLine(vscode_1.l10n.t("    >>> {0}   … exited with signal: {1}", cmdOutputMessage, signal));
                }
            });
            child.stdout.on("data", (data) => {
                stdoutData.push(data.toString());
                ShellExecutionHelper.outputDataChunk(this._outputChannel, data, vscode_1.l10n.t("    stdout: "));
            });
            child.stderr.on("data", (data) => {
                ShellExecutionHelper.outputDataChunk(this._outputChannel, data, vscode_1.l10n.t("    stderr: "));
            });
            await child;
