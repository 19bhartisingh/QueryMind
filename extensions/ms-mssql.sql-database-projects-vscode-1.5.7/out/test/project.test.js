...
const projectEntry_1 = require("../src/models/projectEntry");
const projectSuite = process.env.SQLPROJ_TEST_MODE === "1" ? suite.skip : suite;
projectSuite("Project: sqlproj content operations", function () {
    ...
    test("Should read Project from sqlproj", async function () {
        ...
        should(project.databaseReferences[0] instanceof projectEntry_1.SystemDatabaseReferenceProjectEntry).equal(true);
    ...
    test("Should read Project with Project reference from sqlproj", async function () {
        ...
        (project.databaseReferences[0] instanceof projectEntry_1.SqlProjectReferenceProjectEntry).should.be.true();
        project.databaseReferences[1].referenceName.should.containEql(constants.master);
        (project.databaseReferences[1] instanceof projectEntry_1.SystemDatabaseReferenceProjectEntry).should.be.true();
    });
    test("Should throw warning message while reading Project with more than 1 pre-deploy script from sqlproj", async function () {
        const stub = sinon
            .stub(vscode_1.window, "showWarningMessage")
            .returns(Promise.resolve(constants.okString));
    ...
    test("Should perform Folder and SQL object script operations", async function () {
        ...
        await project.addScriptItem(scriptPathTagged, scriptContentsTagged, "externalStreamingJob" /* ItemType.externalStreamingJob */);
        project.folders.length.should.equal(1);
        project.sqlObjectScripts.length.should.equal(2);
        should(project.folders.find((f) => f.relativePath === (0, utils_1.convertSlashesForSqlProj)(folderPath))).not.equal(undefined);
        should(project.sqlObjectScripts.find((f) => f.relativePath === (0, utils_1.convertSlashesForSqlProj)(scriptPath))).not.equal(undefined);
        should(project.sqlObjectScripts.find((f) => f.relativePath === (0, utils_1.convertSlashesForSqlProj)(scriptPathTagged))).not.equal(undefined);
    ...
    test("Should bulk-add scripts to sqlproj with pre-existing scripts on disk", async function () {
        const project = await testUtils.createTestSqlProject(this.test);
        // initial setup
        ...
        await fs_1.promises.writeFile(tablePath, "CREATE TABLE [MyTable] ([Name] [nvarchar(50)");
        const viewPath = path.join(project.projectFolderPath, "MyView.sql");
        await fs_1.promises.writeFile(viewPath, "CREATE VIEW [MyView] AS SELECT * FROM [MyTable]");
        // add to project
    ...
    test("Should throw error while adding folders and SQL object scripts to sqlproj when a file does not exist on disk", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openProjectFileBaseline);
        const project = await testUtils.createTestSqlProject(this.test);
        let list = [];
        let testFolderPath = await testUtils.createDummyFileStructure(this.test, true, list, path.dirname(projFilePath));
        const nonexistentFile = path.join(testFolderPath, "nonexistentFile.sql");
        list.push(vscode_1.Uri.file(nonexistentFile));
    ...
    test("Should perform pre-deployment script operations", async function () {
        ...
        (await (0, utils_1.exists)(absolutePath)).should.be.false("inital state");
        // add new
        await project.addScriptItem(relativePath, fileContents, "preDeployScript" /* ItemType.preDeployScript */);
        project.preDeployScripts.length.should.equal(1);
        (await (0, utils_1.exists)(absolutePath)).should.be.true("add new");
        // read
        ...
        (await (0, utils_1.exists)(absolutePath)).should.be.true("exclude");
        // add existing
        await project.addScriptItem(relativePath, undefined, "preDeployScript" /* ItemType.preDeployScript */);
        ...
        (await (0, utils_1.exists)(absolutePath)).should.be.false("delete");
    });
    test("Should show information messages when adding more than one pre/post deployment scripts to sqlproj", async function () {
        const stub = sinon.stub(vscode_1.window, "showInformationMessage").returns(Promise.resolve());
        const project = await testUtils.createTestSqlProject(this.test);
        ...
        const fileContents = "SELECT 7";
        await project.addScriptItem(preDeploymentScriptFilePath, fileContents, "preDeployScript" /* ItemType.preDeployScript */);
        await project.addScriptItem(postDeploymentScriptFilePath, fileContents, "postDeployScript" /* ItemType.postDeployScript */);
        stub.notCalled.should.be.true("showInformationMessage should not have been called");
        await project.addScriptItem(preDeploymentScriptFilePath2, fileContents, "preDeployScript" /* ItemType.preDeployScript */);
        ...
        stub.resetHistory();
        await project.addScriptItem(postDeploymentScriptFilePath2, fileContents, "postDeployScript" /* ItemType.postDeployScript */);
    ...
    // TODO: move to DacFx once script contents supported
    test("Should not overwrite existing files", async function () {
        // Create new sqlproj
        ...
        let project = await project_1.Project.openProject(projFilePath);
        // Add a file entry to the project with explicit content
        let existingFileUri = fileList[3];
        let fileStats = await fs_1.promises.stat(existingFileUri.fsPath);
        ...
        await testUtils.shouldThrowSpecificError(async () => await project.addScriptItem(relativePath, "Hello World!"), `A file with the name '${path.parse(relativePath).name}' already exists on disk at this location. Please choose another name.`);
    });
    ...
    test.skip("Should not add folders outside of the project folder", async function () {
        // Create new sqlproj
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        ...
        await testUtils.shouldThrowSpecificError(async () => await project.addFolder(path.dirname(path.dirname(projFilePath))), "Items with absolute path outside project folder are not supported. Please make sure the paths in the project file are relative to project folder.", "Folders outside the project folder should not be added.");
    });
    test("Should handle adding existing items to project", async function () {
        // Create new sqlproj
        const project = await testUtils.createTestSqlProject(this.test);
        ...
        await fs_1.promises.writeFile(sqlFile, "CREATE TABLE T1 (C1 INT)");
        await fs_1.promises.mkdir(path.dirname(txtFile));
        await fs_1.promises.writeFile(txtFile, "Hello World!");
    ...
    test("Should read project properties", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.sqlProjPropertyReadBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        project.sqlProjStyle.should.equal(0 /* ProjectType.SdkStyle */);
        project.outputPath.should.equal(path.join((0, utils_1.getPlatformSafeFileEntryPath)(project.projectFolderPath), (0, utils_1.getPlatformSafeFileEntryPath)("CustomOutputPath\\Dacpacs\\")));
...
projectSuite("Project: sdk style project content operations", function () {
    suiteSetup(async function () {
        await baselines.loadBaselines();
    });
    setup(function () {
    ...
    test("Should exclude pre/post/none deploy scripts correctly", async function () {
        ...
        const project = await project_1.Project.openProject(projFilePath);
        await project.addScriptItem("Script.PreDeployment1.sql", "fake contents", "preDeployScript" /* ItemType.preDeployScript */);
        await project.addScriptItem("Script.PreDeployment2.sql", "fake contents", "preDeployScript" /* ItemType.preDeployScript */);
        await project.addScriptItem("Script.PostDeployment1.sql", "fake contents", "postDeployScript" /* ItemType.postDeployScript */);
    ...
    test("Should handle excluding glob included folders", async function () {
    ...
    test("Should handle excluding folders", async function () {
        const testFolderPath = await testUtils.generateTestFolderPath(this.test);
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openSdkStyleSqlProjectBaseline, testFolderPath);
        await testUtils.createDummyFileStructureWithPrePostDeployScripts(this.test, false, undefined, path.dirname(projFilePath));
        const project = await project_1.Project.openProject(projFilePath);
    ...
    test("Should handle excluding explicitly included folders", async function () {
        const testFolderPath = await testUtils.generateTestFolderPath(this.test);
    ...
    test("Should handle deleting explicitly included folders", async function () {
        const testFolderPath = await testUtils.generateTestFolderPath(this.test);
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openSdkStyleSqlProjectWithFilesSpecifiedBaseline, testFolderPath);
        await testUtils.createDummyFileStructureWithPrePostDeployScripts(this.test, false, undefined, path.dirname(projFilePath));
        const project = await project_1.Project.openProject(projFilePath);
    ...
    test("Should read OutputPath from sqlproj if there is one for SDK-style project", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openSdkStyleSqlProjectBaseline);
        const projFileText = (await fs_1.promises.readFile(projFilePath)).toString();
        // Verify sqlproj has OutputPath
        should(projFileText.includes(constants.OutputPath)).equal(true);
        const project = await project_1.Project.openProject(projFilePath);
        should(project.outputPath).equal(path.join((0, utils_1.getPlatformSafeFileEntryPath)(project.projectFolderPath), (0, utils_1.getPlatformSafeFileEntryPath)("..\\otherFolder")));
        should(project.dacpacOutputPath).equal(path.join((0, utils_1.getPlatformSafeFileEntryPath)(project.projectFolderPath), (0, utils_1.getPlatformSafeFileEntryPath)("..\\otherFolder"), `${project.projectFileName}.dacpac`));
    });
    // TODO: move test to DacFx
    test("Should use default output path if OutputPath is not specified in sqlproj", async function () {
        ...
        const project = await project_1.Project.openProject(projFilePath);
        should(project.outputPath).equal(path.join((0, utils_1.getPlatformSafeFileEntryPath)(project.projectFolderPath), (0, utils_1.getPlatformSafeFileEntryPath)(constants.defaultOutputPath(project.configuration.toString()))) + path.sep);
        should(project.dacpacOutputPath).equal(path.join((0, utils_1.getPlatformSafeFileEntryPath)(project.projectFolderPath), (0, utils_1.getPlatformSafeFileEntryPath)(constants.defaultOutputPath(project.configuration.toString())), `${project.projectFileName}.dacpac`));
    });
});
projectSuite("Project: database references", function () {
    suiteSetup(async function () {
        await baselines.loadBaselines();
    });
    suiteTeardown(async function () {
    ...
    test("Should read database references correctly", async function () {
        ...
        const systemRef = project.databaseReferences.find((r) => r instanceof projectEntry_1.SystemDatabaseReferenceProjectEntry);
        ...
        let projRef = project.databaseReferences.find((r) => r instanceof projectEntry_1.SqlProjectReferenceProjectEntry &&
        ...
        projRef = project.databaseReferences.find((r) => r instanceof projectEntry_1.SqlProjectReferenceProjectEntry && r.referenceName === "OtherProject");
        ...
        let dacpacRef = project.databaseReferences.find((r) => r instanceof projectEntry_1.DacpacReferenceProjectEntry && r.referenceName === "ReferencedDacpac");
        ...
        dacpacRef = project.databaseReferences.find((r) => r instanceof projectEntry_1.DacpacReferenceProjectEntry && r.referenceName === "OtherDacpac");
    ...
    test("Should delete database references correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.databaseReferencesReadBaseline);
        const project = await project_1.Project.openProject(projFilePath);
    ...
    test("Should add system database artifact reference correctly", async function () {
        let project = await testUtils.createTestSqlProject(this.test);
        const msdbRefSettings = {
            databaseVariableLiteralValue: (0, utils_1.systemDatabaseToString)(1 /* SystemDatabase.MSDB */),
            systemDb: 1 /* SystemDatabase.MSDB */,
            suppressMissingDependenciesErrors: true,
            systemDbReferenceType: 0 /* SystemDbReferenceType.ArtifactReference */,
        };
        ...
        const projFileText = (await fs_1.promises.readFile(project.projectFilePath)).toString();
        projFileText.should.containEql('<ArtifactReference Include="$(SystemDacpacsLocation)');
    });
    test("Should add system database package reference correctly", async function () {
        let project = await testUtils.createTestSqlProject(this.test);
        const msdbRefSettings = {
            databaseVariableLiteralValue: (0, utils_1.systemDatabaseToString)(1 /* SystemDatabase.MSDB */),
            systemDb: 1 /* SystemDatabase.MSDB */,
            suppressMissingDependenciesErrors: true,
            systemDbReferenceType: 1 /* SystemDbReferenceType.PackageReference */,
        };
        await project.addSystemDatabaseReference(msdbRefSettings);
        project.databaseReferences.length.should.equal(1, "There should be one database reference after adding a reference to msdb");
        project.databaseReferences[0].referenceName.should.equal(msdbRefSettings.databaseVariableLiteralValue, "databaseName");
    ...
    test("Should add a dacpac reference to the same database correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference in the same database
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        await project.addDatabaseReference({
            dacpacFileLocation: vscode_1.Uri.file("test1.dacpac"),
            suppressMissingDependenciesErrors: true,
        });
    ...
    test("Should add a dacpac reference to a different database in the same server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        // add database reference to a different database on the same server
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        await project.addDatabaseReference({
            dacpacFileLocation: vscode_1.Uri.file("test2.dacpac"),
    ...
    test("Should add a dacpac reference to a different database in a different server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        // add database reference to a different database on a different server
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        await project.addDatabaseReference({
            dacpacFileLocation: vscode_1.Uri.file("test3.dacpac"),
    ...
    test("Should add a project reference to the same database correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        ...
        await project.addProjectReference({
            ...
            projectGuid: "",
            projectRelativePath: vscode_1.Uri.file(path.join("..", "project1", "project1.sqlproj")),
            suppressMissingDependenciesErrors: false,
        });
    ...
    test("Should add a project reference to a different database in the same server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
    ...
    test("Should add a project reference to a different database in a different server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference to a different database on a different server
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
    ...
    test("Should add a nupkg reference to the same database correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference to the same database
        should(project.sqlProjStyle).equal(0 /* ProjectType.SdkStyle */, "Project should be SDK-style");
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        should(project.sqlCmdVariables.size).equal(0, `There should be no sqlcmd variables to start with. Actual: ${project.sqlCmdVariables.size}`);
    ...
    test("Should add a nupkg reference to a different database in the same server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference to a different database on the same different server
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
    ...
    test("Should add a nupkg reference to a different database in a different server correctly", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference to a different database on a different server
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
    ...
    test("Should throw an error trying to add a nupkg reference to legacy style project", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        // add database reference to the same database
        should(project.sqlProjStyle).equal(1 /* ProjectType.LegacyStyle */, "Project should be legacy-style");
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        should(project.sqlCmdVariables.size).equal(0, `There should be no sqlcmd variables to start with. Actual: ${project.sqlCmdVariables.size}`);
    ...
    test("Should not allow adding duplicate dacpac references", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        const dacpacReference = {
            dacpacFileLocation: vscode_1.Uri.file("test.dacpac"),
            suppressMissingDependenciesErrors: false,
        };
    ...
    test("Should not allow adding duplicate system database references", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        const systemDbReference = {
            databaseVariableLiteralValue: (0, utils_1.systemDatabaseToString)(0 /* SystemDatabase.Master */),
            systemDb: 0 /* SystemDatabase.Master */,
            suppressMissingDependenciesErrors: false,
            systemDbReferenceType: 0 /* SystemDbReferenceType.ArtifactReference */,
        };
        await project.addSystemDatabaseReference(systemDbReference);
        project = await project_1.Project.openProject(projFilePath);
    ...
    test("Should not allow adding duplicate project references", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        const projectReference = {
            ...
            projectGuid: "",
            projectRelativePath: vscode_1.Uri.file("testProject.sqlproj"),
            suppressMissingDependenciesErrors: false,
        };
    ...
    test("Should not allow adding duplicate nupkg references", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        const nupkgReference = {
            packageName: "testPackage",
    ...
    test("Should handle trying to add duplicate database references when slashes are different direction", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
        should(project.databaseReferences.length).equal(0, "There should be no database references to start with");
        const projectReference = {
            projectName: "testProject",
            projectGuid: "",
            projectRelativePath: vscode_1.Uri.file("testFolder/testProject.sqlproj"),
            suppressMissingDependenciesErrors: false,
        };
        await project.addProjectReference(projectReference);
        should(project.databaseReferences.length).equal(1, "There should be one database reference after adding a reference to testProject.sqlproj");
        ...
        projectReference.projectRelativePath = vscode_1.Uri.file("testFolder\\testProject.sqlproj");
        await testUtils.shouldThrowSpecificError(async () => await project.addProjectReference(projectReference), constants.databaseReferenceAlreadyExists);
        should(project.databaseReferences.length).equal(1, "There should be one database reference after trying to add a reference to testProject again");
    });
    test("Should update sqlcmd variable values if value changes", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        const project = await project_1.Project.openProject(projFilePath);
...
projectSuite("Project: add SQLCMD Variables", function () {
    suiteSetup(async function () {
        await baselines.loadBaselines();
    });
    suiteTeardown(async function () {
    ...
    test("Should update .sqlproj with new sqlcmd variables", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openProjectFileBaseline);
        let project = await project_1.Project.openProject(projFilePath);
...
projectSuite("Project: publish profiles", function () {
    suiteSetup(async function () {
        await baselines.loadBaselines();
    });
    suiteTeardown(async function () {
...
projectSuite("Project: properties", function () {
    ...
    test("Should read target database version", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openProjectFileBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        should(project.getProjectTargetVersion()).equal("150");
    });
    test("Should throw on missing target database version", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.sqlProjectMissingVersionBaseline);
        await testUtils.shouldThrowSpecificError(async () => await project_1.Project.openProject(projFilePath), "Error: No target platform defined.  Missing <DSP> node.");
    });
    test("Should throw on invalid target database version", async function () {
    ...
    test("Should read default database collation", async function () {
    ...
    test("Should return default value when database collation is not specified", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.newProjectFileBaseline);
        const project = await project_1.Project.openProject(projFilePath);
    ...
    test.skip("Should throw on invalid default database collation", async function () {
    ...
    test("Should add database source to project property", async function () {
        const project = await testUtils.createTestSqlProject(this.test);
        // Should add a single database source
        await project.addDatabaseSource("test1");
        let databaseSourceItems = project.getDatabaseSourceValues();
    ...
    test("Should remove database source from project property", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.sqlProjectInvalidCollationBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        await project.addDatabaseSource("test1");
        await project.addDatabaseSource("test2");
    ...
    test("Should throw error when adding or removing database source that contains semicolon", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.sqlProjectInvalidCollationBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        const semicolon = ";";
        await testUtils.shouldThrowSpecificError(async () => await project.addDatabaseSource(semicolon), constants.invalidProjectPropertyValueProvided(semicolon));
        await testUtils.shouldThrowSpecificError(async () => await project.removeDatabaseSource(semicolon), constants.invalidProjectPropertyValueProvided(semicolon));
    });
});
projectSuite("Project: round trip updates", function () {
    suiteSetup(async function () {
        await baselines.loadBaselines();
    });
    setup(function () {
    ...
    test("Should update SSDT project to work in ADS", async function () {
        await testUpdateInRoundTrip(this.test, baselines.SSDTProjectFileBaseline);
    });
    test.skip("Should update SSDT project with new system database references", async function () {
        await testUpdateInRoundTrip(this.test, baselines.SSDTUpdatedProjectBaseline);
    });
    test("Should update SSDT project to work in ADS handling pre-existing targets", async function () {
        await testUpdateInRoundTrip(this.test, baselines.SSDTProjectBaselineWithBeforeBuildTarget);
    });
    test("Should not update project and no backup file should be created when prompt to update project is rejected", async function () {
        sinon.stub(vscode_1.window, "showWarningMessage").returns(Promise.resolve(constants.noString));
        // setup test files
        const folderPath = await testUtils.generateTestFolderPath(this.test);
        const sqlProjPath = await testUtils.createTestSqlProjFile(this.test, baselines.SSDTProjectFileBaseline, folderPath);
        const originalSqlProjContents = (await fs_1.promises.readFile(sqlProjPath)).toString();
        // validate original state
        let project = await project_1.Project.openProject(sqlProjPath, false);
        ...
        project = await project_1.Project.openProject(sqlProjPath, true);
        ...
        (await (0, utils_1.exists)(sqlProjPath + "_backup")).should.be.false("backup file should not be generated");
        const newSqlProjContents = (await fs_1.promises.readFile(sqlProjPath)).toString();
    ...
    test("Should not show warning message for non-SSDT projects that have the additional information for Build", async function () {
        // setup test files
        const folderPath = await testUtils.generateTestFolderPath(this.test);
        ...
        await project_1.Project.openProject(vscode_1.Uri.file(sqlProjPath).fsPath); // no error thrown
    });
    test("Should not show update project warning message when opening sdk style project using Sdk node", async function () {
        await shouldNotShowUpdateWarning(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
    });
    test("Should not show update project warning message when opening sdk style project using Project node with Sdk attribute", async function () {
        await shouldNotShowUpdateWarning(this.test, baselines.newSdkStyleProjectSdkProjectAttributeBaseline);
    });
    test("Should not show update project warning message when opening sdk style project using Import node with Sdk attribute", async function () {
        await shouldNotShowUpdateWarning(this.test, baselines.newStyleProjectSdkImportAttributeBaseline);
    });
    async function shouldNotShowUpdateWarning(test, baselineFile) {
        // setup test files
        ...
        const spy = sinon.spy(vscode_1.window, "showWarningMessage");
        const project = await project_1.Project.openProject(vscode_1.Uri.file(sqlProjPath).fsPath);
    ...
    test("Should filter out glob patterns from None items", async function () {
        const projFilePath = await testUtils.createTestSqlProjFile(this.test, baselines.openSdkStyleSqlProjectBaseline);
        const project = await project_1.Project.openProject(projFilePath);
        ...
        const hasGlobPattern = project.noneDeployScripts.some((f) => f.relativePath.includes("*") ||
            f.relativePath.includes("?") ||
            f.relativePath.includes("["));
        should(hasGlobPattern).be.false("None items should not contain glob patterns with *, ?, or [");
    });
});
async function testUpdateInRoundTrip(test, fileBeforeupdate) {
    const projFilePath = await testUtils.createTestSqlProjFile(test, fileBeforeupdate);
    const project = await project_1.Project.openProject(projFilePath); // project gets updated if needed in openProject()
    ...
    (await (0, utils_1.exists)(projFilePath + "_backup")).should.be.true("Backup file should have been generated before the project was updated");
    sinon.restore();
}
