...
const databaseProjectTreeViewProvider_1 = require("../src/controllers/databaseProjectTreeViewProvider");
const projectController_1 = require("../src/controllers/projectController");
const fs_1 = require("fs");
const testContext_1 = require("./testContext");
const project_1 = require("../src/models/project");
const projectTreeItem_1 = require("../src/models/tree/projectTreeItem");
let testContext;
const templatesPath = testUtils.getTemplatesRootPath();
suite("ProjectsController", function () {
    ...
    setup(function () {
        ...
        sinon.stub(utils, "getSqlProjectsService").resolves({
            ...
            getProjectProperties: async () => ({
                ...
                projectStyle: 0 /* mssql.ProjectType.SdkStyle */,
            }),
    ...
    suite("project controller operations", function () {
        suite("Project file operations and prompting", function () {
            test("Should create new sqlproj file with correct specified target platform", async function () {
                ...
                const projTargetPlatform = "Azure SQL Database" /* SqlTargetPlatform.sqlAzure */; // default is SQL Server 2022
            ...
            test.skip("Should create new edge project with expected template files", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const projFileDir = await testUtils.generateTestFolderPath(this.test);
                const projFilePath = await projController.createNewProject({
                    newProjName: "TestProjectName",
            ...
            test("Should return silently when no SQL object name provided in prompts", async function () {
                for (const name of ["", "    ", undefined]) {
                    sinon.stub(vscode.window, "showInputBox").resolves(name);
                    sinon.stub(utils, "sanitizeStringForFilename").returns("");
                    const showErrorMessageSpy = sinon.spy(vscode.window, "showErrorMessage");
                    const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                    const project = new project_1.Project("FakePath");
                    should(project.sqlObjectScripts.length).equal(0);
                    await projController.addItemPrompt(new project_1.Project("FakePath"), "", {
                        itemType: "script" /* ItemType.script */,
                    });
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should show error if trying to add a file that already exists", async function () {
                ...
                await projController.addItemPrompt(project, "", { itemType: "script" /* ItemType.script */ });
            ...
            test("Should not create file if no itemTypeName is selected", async function () {
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should add existing item", async function () {
                const tableName = "table1";
                sinon.stub(vscode.window, "showInputBox").resolves(tableName);
                sinon.stub(utils, "sanitizeStringForFilename").returns(tableName);
                const spy = sinon.spy(vscode.window, "showErrorMessage");
                ...
                should(project.sqlObjectScripts.length).equal(1, "File should be successfully added");
                // exclude item
                const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(project);
                await projController.exclude(createWorkspaceTreeItem((projTreeRoot.children.find((x) => x.friendlyName === "table1.sql"))));
                // reload project
                project = await project_1.Project.openProject(project.projectFilePath);
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should show error if trying to add a folder that already exists", async function () {
                ...
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                let project = await testUtils.createTestProject(this.test, baselines.newProjectFileBaseline);
                const projectRoot = new projectTreeItem_1.ProjectRootTreeItem(project);
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should be able to add folder with reserved name as long as not at project root", async function () {
                const folderName = "folder1";
                ...
                for (let i in constants.reservedProjectFolders) {
                    // reload project
                    project = await project_1.Project.openProject(project.projectFilePath);
                    await verifyFolderAdded(constants.reservedProjectFolders[i], projController, project, node);
                }
            });
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return default folder for item type only when folder exists", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("schema" /* ItemType.schema */, project), "Schema should return empty when Security folder does not exist").to.equal("");
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("table" /* ItemType.table */, project), "Table should not have a default folder").to.equal("");
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("view" /* ItemType.view */, project), "View should not have a default folder").to.equal("");
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("schema" /* ItemType.schema */, project), "Schema should return Security folder when it exists").to.equal(constants.securityFolderName);
            });
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return empty when no schema folder exists", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const project = await testUtils.createTestProject(this.test, templates.newSqlProjectTemplate);
                // Without schema folder - should return empty string (root level)
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "dbo"), "Should return empty when no dbo folder exists").to.equal("");
                // Add dbo folder to project
                await project.addFolder("dbo");
                // With dbo folder - should return dbo
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "dbo"), "Should return dbo folder when it exists").to.equal("dbo");
            });
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return nested schema/object-type folder when it exists", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const project = await testUtils.createTestProject(this.test, templates.newSqlProjectTemplate);
                // No folders - should return empty
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "sales"), "Should return empty when no Sales folder").to.equal("");
                // Add Sales folder to project
                await project.addFolder("Sales");
                // With Sales folder only - should return Sales folder
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "sales"), "Should return Sales folder when no nested Functions folder").to.equal("Sales");
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "sales"), "Should return Sales/Functions when nested folder exists").to.equal("Sales\\Functions");
                // With dbo schema and no dbo folder - should return empty (place at root)
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("tableValuedFunction" /* ItemType.tableValuedFunction */, project, "dbo"), "Should return empty when dbo folder does not exist").to.equal("");
                // Add dbo folder
                await project.addFolder("dbo");
            ...
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return DatabaseTriggers folder for database trigger regardless of schema", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("databaseTrigger" /* ItemType.databaseTrigger */, project, ""), "Should return empty when DatabaseTriggers folder does not exist").to.equal("");
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("databaseTrigger" /* ItemType.databaseTrigger */, project, ""), "Should return DatabaseTriggers folder").to.equal("DatabaseTriggers");
            });
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return Sequences folder for sequence with root-first priority", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const project = await testUtils.createTestProject(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
                // Without any folders - should return empty (root level)
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("sequence" /* ItemType.sequence */, project, "dbo"), "Should return empty when no folders exist").to.equal("");
                // Add dbo folder only (no Sequences folders)
                await project.addFolder("dbo");
                // With only dbo folder - should return dbo (schema folder)
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("sequence" /* ItemType.sequence */, project, "dbo"), "Should return dbo folder when it exists").to.equal("dbo");
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("sequence" /* ItemType.sequence */, project, "dbo"), "Should return root Sequences folder").to.equal("Sequences");
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("sequence" /* ItemType.sequence */, project, "dbo"), "Should still return root Sequences even when dbo/Sequences exists").to.equal("Sequences");
            });
            // TODO: SAI - Mock returns empty folders, needs service integration
            test.skip("Should return schema/Sequences folder when only nested folder exists", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const project = await testUtils.createTestProject(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
                ...
                (0, chai_1.expect)(projController.getDefaultFolderForItemType("sequence" /* ItemType.sequence */, project, "dbo"), "Should return dbo/Sequences when no root Sequences exists").to.equal("dbo\\Sequences");
            });
            test("Should parse schema and object name from user input", function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                // Test schema-qualified name
                let result = projController.parseSchemaAndObjectName("sales.MyFunction");
                (0, chai_1.expect)(result.schemaName, "Schema should be parsed correctly").to.equal("sales");
                (0, chai_1.expect)(result.objectName, "Object name should be parsed correctly").to.equal("MyFunction");
                // Test simple name (no schema)
                result = projController.parseSchemaAndObjectName("MyFunction");
                (0, chai_1.expect)(result.schemaName, "Default schema should be dbo").to.equal("dbo");
                (0, chai_1.expect)(result.objectName, "Object name should remain unchanged").to.equal("MyFunction");
                // Test edge case: leading dot
                result = projController.parseSchemaAndObjectName(".MyFunction");
                (0, chai_1.expect)(result.schemaName, "Leading dot should not be treated as schema").to.equal("dbo");
                (0, chai_1.expect)(result.objectName, "Full string should be the object name").to.equal(".MyFunction");
                // Test edge case: trailing dot
                result = projController.parseSchemaAndObjectName("MyFunction.");
                (0, chai_1.expect)(result.schemaName, "Trailing dot should not be treated as schema").to.equal("dbo");
                (0, chai_1.expect)(result.objectName, "Full string should be the object name").to.equal("MyFunction.");
            });
            test("Should create .vscode/tasks.json at workspace level with isDefault=true when configureDefaultBuild is true", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const projFileDir = await testUtils.generateTestFolderPath(this.test);
                ...
                await project_1.Project.openProject(projFilePath);
                ...
                (0, chai_1.expect)(exists, ".vscode/tasks.json should be created at workspace level when configureDefaultBuild is true").to.be.true;
                // If exists, check if isDefault is true in any build task
                if (exists) {
                    const tasksJsonContent = await fs_1.promises.readFile(tasksJsonPath, "utf-8");
                    const tasksJson = JSON.parse(tasksJsonContent);
                    (0, chai_1.expect)(tasksJson.tasks, "tasks should be an array")
                        .to.be.an("array")
                        .with.lengthOf(1);
                    const task = tasksJson.tasks[0];
                    (0, chai_1.expect)(task.group, "task group should be defined").to.not.be.undefined;
                    (0, chai_1.expect)(task.group.isDefault, "The build task should have isDefault: true (boolean)").to.equal(true);
                }
            });
            test("Should merge SQL build task into existing tasks.json when it already exists at workspace level", async function () {
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const projFileDir = await testUtils.generateTestFolderPath(this.test);
                // Create existing tasks.json with a different task
                const vscodeFolder = path.join(projFileDir, ".vscode");
                await fs_1.promises.mkdir(vscodeFolder, { recursive: true });
                ...
                await fs_1.promises.writeFile(path.join(vscodeFolder, "tasks.json"), JSON.stringify(existingTasksJson, null, "\t"));
                ...
                (0, chai_1.expect)(tasksJson.tasks, "tasks should be an array")
                    .to.be.an("array")
                    .with.lengthOf(2);
                (0, chai_1.expect)(tasksJson.tasks[0].label, "Existing task should be preserved").to.equal("Existing Task");
                (0, chai_1.expect)(tasksJson.tasks[1].label, "SQL build task should be added").to.equal(constants.getSqlProjectBuildTaskLabel("TestProjectWithTasks"));
                // Assert: notification was shown
                (0, chai_1.expect)(showInfoSpy.calledWith(constants.updatingExistingTasksJson), "Should show notification when updating existing tasks.json").to.be.true;
            });
            async function verifyFolderAdded(folderName, projController, project, node) {
            ...
            async function verifyFolderNotAdded(folderName, projController, project, node) {
            ...
            test.skip("Should delete nested ProjectEntry from node", async function () {
                ...
                proj = await project_1.Project.openProject(proj.projectFilePath); // reload edited sqlproj from disk
                // confirm result
            ...
            test.skip("Should delete database references", async function () {
                ...
                sinon
                    ...
                    .returns(Promise.resolve(constants.yesString));
                ...
                const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(proj);
                ...
                // confirm result
                // reload project
                proj = await project_1.Project.openProject(proj.projectFilePath);
                should(proj.databaseReferences.length).equal(0, "All database references should have been deleted");
            });
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should exclude nested ProjectEntry from node", async function () {
                let proj = await testUtils.createTestSqlProject(this.test);
                const setupResult = await setupDeleteExcludeTest(proj);
                const scriptEntry = setupResult[0], projTreeRoot = setupResult[1], preDeployEntry = setupResult[2], postDeployEntry = setupResult[3], noneEntry = setupResult[4];
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                await projController.exclude(createWorkspaceTreeItem((projTreeRoot.children.find((x) => x.friendlyName === "UpperFolder")
                    .children[0])) /* LowerFolder */);
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should exclude a folder", async function () {
                let proj = await testUtils.createTestSqlProject(this.test);
            ...
            // TODO: move test to DacFx and fix delete
            test.skip("Should delete folders with excluded items", async function () {
                let proj = await testUtils.createTestProject(this.test, templates.newSqlProjectTemplate);
                const setupResult = await setupDeleteExcludeTest(proj);
                ...
                await projController.exclude(createWorkspaceTreeItem((lowerFolder.children.find((x) => x.friendlyName === "someScript.sql"))));
                await projController.exclude(createWorkspaceTreeItem((lowerFolder.children.find((x) => x.friendlyName === "someOtherScript.sql"))));
                // Delete UpperFolder
                await projController.delete(createWorkspaceTreeItem((projTreeRoot.children.find((x) => x.friendlyName === "UpperFolder"))));
            ...
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should reload correctly after changing sqlproj file", async function () {
                // create project
                ...
                const treeProvider = new databaseProjectTreeViewProvider_1.SqlDatabaseProjectTreeViewProvider();
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                let project = await project_1.Project.openProject(vscode.Uri.file(sqlProjPath).fsPath);
                treeProvider.load([project]);
                // change the sql project file
                await fs_1.promises.writeFile(sqlProjPath, baselines.newProjectFileWithScriptBaseline);
                should(project.sqlObjectScripts.length).equal(0);
                // call reload project
                const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(project);
                ...
                // reload project
                project = await project_1.Project.openProject(sqlProjPath, false, true);
                treeProvider.load([project]);
                // check that the new project is in the tree
                should(project.sqlObjectScripts.length).equal(1);
                should(treeProvider
                    .getChildren()[0]
                    .children.find((c) => c.friendlyName === "Script1.sql")).not.equal(undefined);
            });
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should be able to add pre deploy and post deploy script", async function () {
                ...
                await projController.addItemPrompt(project, "", {
                    itemType: "preDeployScript" /* ItemType.preDeployScript */,
                });
                ...
                await projController.addItemPrompt(project, "", {
                    itemType: "postDeployScript" /* ItemType.postDeployScript */,
                });
                should(project.postDeployScripts.length).equal(1, "Post deploy script should be successfully added");
            });
            // TODO: SAI - Mock returns empty data, needs service integration
            test.skip("Should be able to add publish profile", async function () {
                ...
                await projController.addItemPrompt(project, "", {
                    itemType: "publishProfile" /* ItemType.publishProfile */,
                });
            ...
            test.skip("Should change target platform", async function () {
                sinon
                    .stub(vscode.window, "showQuickPick")
                    .resolves({ label: "Azure SQL Database" /* SqlTargetPlatform.sqlAzure */ });
                const projController = new projectController_1.ProjectsController(testContext.outputChannel);
                const sqlProjPath = await testUtils.createTestSqlProjFile(this.test, baselines.openProjectFileBaseline);
                const project = await project_1.Project.openProject(sqlProjPath);
                should(project.getProjectTargetVersion()).equal(constants.targetPlatformToVersion.get("SQL Server 2019" /* SqlTargetPlatform.sqlServer2019 */));
                ...
                should(project.getProjectTargetVersion()).equal(constants.targetPlatformToVersion.get("Azure SQL Database" /* SqlTargetPlatform.sqlAzure */));
            });
        });
        suite("Publishing and script generation", function () {
            const sandbox = sinon.createSandbox();
            teardown(function () {
                sandbox.restore();
            });
            test("publishProject should invoke mssql.publishDatabaseProject command with correct project path", async function () {
                ...
                (0, chai_1.expect)(executeCommandStub.calledOnce, "executeCommand should be called exactly once").to.be.true;
                (0, chai_1.expect)(executeCommandStub.firstCall.args[0]).to.equal(constants.mssqlPublishProjectCommand, "should invoke the mssql publish project command");
                (0, chai_1.expect)(executeCommandStub.firstCall.args[1]).to.equal(expectedProjectPath, "should pass the correct project file path");
            });
        });
    });
    suite("Create project from database operations and dialog", function () {
        teardown(() => {
            sinon.restore();
        });
        // TODO: SAI - Test expects specific file structure, needs review
        test.skip("Should create list of all files and folders correctly", async function () {
            ...
            fileList
                .filter((x) => path.extname(x.fsPath) !== constants.sqlFileExtension)
                .length.should.equal(0, "number of non-.sql files");
        });
        test("Should error out for inaccessible path", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
        ...
        test("Should set model filePath correctly for ExtractType = File", async function () {
            ...
            let model = {
                ...
                filePath: folderPath,
                version: "1.0.0.0",
                extractTarget: 1 /* mssql.ExtractTarget.file */,
                sdkStyle: false,
            };
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        test("Should set model filePath correctly for ExtractType = Schema/Object Type", async function () {
            let folderPath = await testUtils.generateTestFolderPath(this.test);
            let projectName = "My Project";
            let importPath;
            let model = {
    ...
    suite("Add database reference", function () {
        test.skip("Should not allow adding circular project references", async function () {
            ...
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            const project1 = await project_1.Project.openProject(vscode.Uri.file(projPath1).fsPath);
            const project2 = await project_1.Project.openProject(vscode.Uri.file(projPath2).fsPath);
            const showErrorMessageSpy = sinon.spy(vscode.window, "showErrorMessage");
            const dataWorkspaceMock = TypeMoq.Mock.ofType();
            dataWorkspaceMock
                .setup((x) => x.getProjectsInWorkspace(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
            ...
            sinon.stub(utils, "getDacFxService").returns({
                parseTSqlScript: (_, __) => {
            ...
            await projController.addDatabaseReferenceCallback(project1, {
                ...
                suppressMissingDependenciesErrors: false,
            }, { treeDataProvider: new databaseProjectTreeViewProvider_1.SqlDatabaseProjectTreeViewProvider(), element: undefined });
            should(showErrorMessageSpy.notCalled).be.true("showErrorMessage should not have been called");
        ...
        test.skip("Should add dacpac references as relative paths", async function () {
            ...
            await projController.addDatabaseReferenceCallback(project1, {
                databaseName: this.databaseNameTextbox?.value,
            ...
            let projFileText = (await fs_1.promises.readFile(projFilePath)).toString();
            ...
            projFileText = (await fs_1.promises.readFile(projFilePath)).toString();
    ...
    suite("AutoRest generation", function () {
        // skipping for now because this feature is hidden under preview flag
        test.skip("Should create project from autorest-generated files", async function () {
            ...
            let fileList = [];
            const projController = TypeMoq.Mock.ofType(projectController_1.ProjectsController);
            projController.callBase = true;
            projController.setup((x) => x.selectAutorestSpecFile()).returns(async () => specName);
            projController
                .setup((x) => x.selectAutorestProjectLocation(TypeMoq.It.isAny(), undefined))
                .returns(async () => {
                await fs_1.promises.mkdir(newProjFolder);
                return {
            ...
            projController
                .setup((x) => x.generateAutorestFiles(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
                .returns(async () => {
            ...
            projController
                .setup((x) => x.promptForAutorestProjectName(TypeMoq.It.isAny()))
                .returns(async () => renamedProjectName);
            projController
                .setup((x) => x.openProjectInWorkspace(TypeMoq.It.isAny()))
                .returns(async () => { });
            ...
            should(project.sqlObjectScripts.filter((f) => f.type === 0 /* EntryType.File */).length).equal(expectedScripts.length, "Unexpected number of scripts in project");
            const expectedFolders = fileList.filter((f) => path.extname(f.fsPath) === "" &&
                f.fsPath.toUpperCase() !== newProjFolder.toUpperCase());
            should(project.sqlObjectScripts.filter((f) => f.type === 1 /* EntryType.Folder */).length).equal(expectedFolders.length, "Unexpected number of folders in project");
        });
    });
    suite("Move file", function () {
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should move a file to another folder", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
            sinon
                .stub(vscode.window, "showWarningMessage")
                .returns(Promise.resolve(constants.move));
            ...
            proj = await project_1.Project.openProject(proj.projectFilePath);
            should(proj.sqlObjectScripts.find((f) => f.relativePath === "UpperFolder\\script1.sql") !==
                undefined).be.true("The file path should have been updated");
            should(await utils.exists(path.join(proj.projectFolderPath, "UpperFolder", "script1.sql"))).be.true("The moved file should exist");
        });
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should move a folder to another folder", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
            sinon
                .stub(vscode.window, "showWarningMessage")
                .returns(Promise.resolve(constants.move));
        ...
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should not allow moving a file to Database References or SQLCMD folder", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
            sinon
                .stub(vscode.window, "showWarningMessage")
                .returns(Promise.resolve(constants.move));
            ...
            for (const folder of foldersToTest) {
                ...
                proj = await project_1.Project.openProject(proj.projectFilePath);
                should(proj.sqlObjectScripts.find((f) => f.relativePath === "script1.sql") !==
                    undefined).be.true(`The file path should not have been updated when trying to move script1.sql to ${folder}`);
                should(spy.notCalled).be.true("showErrorMessage should not have been called.");
                spy.restore();
            }
        });
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should only allow moving files and folders", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
            let proj = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const projTreeRoot = await setupMoveTest(proj);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should not allow moving files between projects", async function () {
            const spy = sinon.spy(vscode.window, "showErrorMessage");
            sinon
                .stub(vscode.window, "showWarningMessage")
                .returns(Promise.resolve(constants.move));
            ...
            proj1 = await project_1.Project.openProject(proj1.projectFilePath);
        ...
        test.skip("Should move a file to project root when project folder name differs from project name", async function () {
            // Arrange
            ...
            await fs_1.promises.rename(originalProjectFolder, newProjectFolder);
            ...
            proj = await project_1.Project.openProject(newProjectPath);
            const projTreeRoot = await setupMoveTest(proj);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            // Get the file to move from nested folder
            const upperFolder = projTreeRoot.children.find((x) => x.friendlyName === "UpperFolder");
            const lowerFolder = upperFolder.children.find((x) => x.friendlyName === "LowerFolder");
            const sqlFileNode = lowerFolder.children.find((x) => x.friendlyName === "someScript.sql");
            const projectRootWorkspaceTreeItem = createWorkspaceTreeItem(projTreeRoot);
            // Act - Move the file from UpperFolder/LowerFolder to the project root
            await projController.moveFile(vscode.Uri.file(proj.projectFilePath), sqlFileNode, projectRootWorkspaceTreeItem);
            // Assert
            (0, chai_1.expect)(errorSpy.notCalled, "showErrorMessage should not have been called").to.be.true;
            // Reload project and verify file was moved to the root
            proj = await project_1.Project.openProject(newProjectPath);
            const movedFile = proj.sqlObjectScripts.find((f) => f.relativePath === "someScript.sql");
            (0, chai_1.expect)(movedFile, "The file path should have been updated to the project root").to.not
                .be.undefined;
            const fileExistsAtRoot = await utils.exists(path.join(proj.projectFolderPath, "someScript.sql"));
            (0, chai_1.expect)(fileExistsAtRoot, "The moved file should exist at the root").to.be.true;
            const fileExistsAtOldLocation = await utils.exists(path.join(proj.projectFolderPath, "UpperFolder", "LowerFolder", "someScript.sql"));
            (0, chai_1.expect)(fileExistsAtOldLocation, "The file should not exist at the old location").to.be
                .false;
        });
    });
    suite("Rename file", function () {
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should not do anything if no new name is provided", async function () {
        ...
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should rename a sql object file", async function () {
        ...
        // TODO: SAI - getProjectFromContext fails, needs service integration
        test.skip("Should rename a pre and post deploy script", async function () {
            let proj = await testUtils.createTestProject(this.test, baselines.newSdkStyleProjectSdkNodeBaseline);
            await proj.addScriptItem("Script.PreDeployment1.sql", "pre-deployment stuff", "preDeployScript" /* ItemType.preDeployScript */);
            await proj.addScriptItem("Script.PostDeployment1.sql", "post-deployment stuff", "postDeployScript" /* ItemType.postDeployScript */);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(proj);
            // try to rename a file from the root folder
            ...
            // reload project and verify files were renamed
            proj = await project_1.Project.openProject(proj.projectFilePath);
            should(proj.preDeployScripts.find((f) => f.relativePath === "predeployNewName.sql") !==
                undefined).be.true("The pre deploy script file path should have been updated");
            ...
            should(proj.postDeployScripts.find((f) => f.relativePath === "postdeployNewName.sql") !==
                undefined).be.true("The post deploy script file path should have been updated");
        ...
        // TODO: SAI - Mock returns empty data, needs service integration
        test.skip("Should rename a folder", async function () {
            let proj = await testUtils.createTestSqlProject(this.test);
            await proj.addScriptItem("SomeFolder/MyTable.sql", "CREATE TABLE [NotARealTable]");
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(proj);
    ...
    suite("SqlCmd Variables", function () {
        // TODO: SAI - Mock returns empty sqlcmd variables, needs service integration
        test.skip("Should delete sqlcmd variable", async function () {
            ...
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            const projRoot = new projectTreeItem_1.ProjectRootTreeItem(project);
            should(project.sqlCmdVariables.size).equal(2, "The project should start with 2 sqlcmd variables");
            sinon.stub(vscode.window, "showWarningMessage").returns(Promise.resolve("Cancel"));
            ...
            sinon.restore();
            sinon.stub(vscode.window, "showWarningMessage").returns(Promise.resolve("Yes"));
        ...
        // TODO: SAI - Mock returns empty sqlcmd variables, needs service integration
        test.skip("Should add sqlcmd variable", async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        // TODO: SAI - Mock returns empty sqlcmd variables, needs service integration
        test.skip("Should add sqlcmd variable without DefaultValue", async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            ...
            infoMessageStub.onFirstCall().returns(Promise.resolve(constants.noString));
            await projController.addSqlCmdVariable(createWorkspaceTreeItem(projRoot.children.find((x) => x.friendlyName === constants.sqlcmdVariablesNodeName)));
            // reload project
            project = await project_1.Project.openProject(project.projectFilePath);
            ...
            inputBoxStub.onFirstCall().resolves("newVariable");
            inputBoxStub.onSecondCall().resolves(undefined);
            infoMessageStub.onSecondCall().returns(Promise.resolve(constants.yesString));
            await projController.addSqlCmdVariable(createWorkspaceTreeItem(projRoot.children.find((x) => x.friendlyName === constants.sqlcmdVariablesNodeName)));
            // reload project
            project = await project_1.Project.openProject(project.projectFilePath);
        ...
        // TODO: SAI - Mock returns empty sqlcmd variables, needs service integration
        test.skip("Should update sqlcmd variable", async function () {
            let project = await testUtils.createTestProject(this.test, baselines.openSdkStyleSqlProjectBaseline);
            const sqlProjectsService = await utils.getSqlProjectsService();
            await sqlProjectsService.openProject(project.projectFilePath);
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
        ...
        // TODO: SAI - Mock returns empty data, needs service integration
        test.skip("Should remove file extensions from user input when creating files", async function () {
            const projController = new projectController_1.ProjectsController(testContext.outputChannel);
            let project = await testUtils.createTestProject(this.test, baselines.newProjectFileBaseline);
...
async function setupDeleteExcludeTest(proj) {
    ...
    const preDeployEntry = await proj.addScriptItem("Script.PreDeployment1.sql", "pre-deployment stuff", "preDeployScript" /* ItemType.preDeployScript */);
    const noneEntry = await proj.addScriptItem("Script.PreDeployment2.sql", "more pre-deployment stuff", "preDeployScript" /* ItemType.preDeployScript */);
    const postDeployEntry = await proj.addScriptItem("Script.PostDeployment1.sql", "post-deployment stuff", "postDeployScript" /* ItemType.postDeployScript */);
    const projTreeRoot = new projectTreeItem_1.ProjectRootTreeItem(proj);
    sinon
        .stub(vscode.window, "showWarningMessage")
        .returns(Promise.resolve(constants.yesString));
    ...
    should((await fs_1.promises.readFile(scriptEntry.fsUri.fsPath)).toString()).equal("not a real script");
    return [scriptEntry, projTreeRoot, preDeployEntry, postDeployEntry, noneEntry];
}
async function setupMoveTest(proj) {
    await proj.addFolder("UpperFolder");
    await proj.addFolder("UpperFolder/LowerFolder");
...
function createWorkspaceTreeItem(node) {
    return {
        element: node,
        treeDataProvider: new databaseProjectTreeViewProvider_1.SqlDatabaseProjectTreeViewProvider(),
    };
}
