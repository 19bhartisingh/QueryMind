...
const fileFolderTreeItem_1 = require("../src/models/tree/fileFolderTreeItem");
const projectTreeItem_1 = require("../src/models/tree/projectTreeItem");
const constants_1 = require("../src/common/constants");
suite("Project Tree tests", function () {
    test("Should correctly order tree nodes by type, then by name", function () {
    ...
    test("Should build tree from Project file correctly", function () {
        const root = os.platform() === "win32" ? "Z:\\" : "/";
        ...
        should(tree.children
            .find((x) => x.relativeProjectUri.path === "/TestProj/someFolder")
            ?.children.map((y) => y.relativeProjectUri.path)).deepEqual([
        ...
        should(tree.children
            .find((x) => x.relativeProjectUri.path === "/TestProj/someFolder")
            ?.children.map((y) => y.treeItem.contextValue)).deepEqual([
            constants_1.DatabaseProjectItemType.folder,
            constants_1.DatabaseProjectItemType.folder,
            constants_1.DatabaseProjectItemType.sqlObjectScript,
            constants_1.DatabaseProjectItemType.sqlObjectScript,
    ...
    test("Should be able to parse windows relative path as platform safe path", function () {
        const root = os.platform() === "win32" ? "Z:\\" : "/";
        const proj = new project_1.Project(vscode.Uri.file(`${root}TestProj.sqlproj`).fsPath);
        // nested entries before explicit top-level folder entry
        // also, ordering of files/folders at all levels
    ...
    test("Should be able to parse and include relative paths outside project folder", function () {
