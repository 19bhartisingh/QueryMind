...
suite("Templates", function (): void {
    ...
    test("Should throw error when attempting to use templates before loaded from file", async function (): Promise<void> {
        templates.reset();
        await shouldThrowSpecificError(
            () => templates.get("foobar"),
            "Templates must be loaded from file before attempting to use.",
        );
        await shouldThrowSpecificError(
            () => templates.get("foobar"),
            "Templates must be loaded from file before attempting to use.",
    ...
    test("Should load all templates from files", async function (): Promise<void> {
        ...
        expect(templates.projectScriptTypes().length).to.equal(
            ...
            `Expected ${numScriptObjectTypes} script object types to be loaded`,
        );
        expect(Object.keys(templates.projectScriptTypes()).length).to.equal(
            numScriptObjectTypes,
            `Expected ${numScriptObjectTypes} keys in script object types`,
        );
    ...
    test("Should have Schema item template", async function (): Promise<void> {
        ...
        expect(
            schemaTemplate.friendlyName,
            'Schema template friendly name should be "Schema"',
        ).to.equal("Schema");
        expect(
            schemaTemplate.templateScript,
            "Schema template should contain CREATE SCHEMA",
        ).to.include("CREATE SCHEMA");
        expect(
            schemaTemplate.templateScript,
            "Schema template should contain @@OBJECT_NAME@@ placeholder",
    ...
    test("Should have Stored Procedure item template", async function (): Promise<void> {
        ...
        expect(spTemplate.type, "Stored Procedure template type should match").to.equal(
    ...
    test("Should have Data Source item template", async function (): Promise<void> {
        ...
        expect(dataSourceTemplate.type, "Data Source template type should match").to.equal(
            ItemType.dataSource,
        );
        expect(
            dataSourceTemplate.templateScript,
            "Data Source template should contain CREATE EXTERNAL DATA SOURCE",
        ).to.include("CREATE EXTERNAL DATA SOURCE");
    });
    ...
    test("Should have File Format item template", async function (): Promise<void> {
        ...
        expect(fileFormatTemplate.type, "File Format template type should match").to.equal(
            ItemType.fileFormat,
        );
        expect(
            fileFormatTemplate.templateScript,
            "File Format template should contain CREATE EXTERNAL FILE FORMAT",
        ).to.include("CREATE EXTERNAL FILE FORMAT");
    });
    ...
    test("Should have External Stream item template", async function (): Promise<void> {
        ...
        expect(externalStreamTemplate, "External Stream template should be defined").to.not.be
            .undefined;
        expect(externalStreamTemplate.type, "External Stream template type should match").to.equal(
            ItemType.externalStream,
        );
        expect(
            externalStreamTemplate.templateScript,
            "External Stream template should contain CREATE EXTERNAL STREAM",
        ).to.include("CREATE EXTERNAL STREAM");
    });
    ...
    test("Should have External Streaming Job item template", async function (): Promise<void> {
        ...
        expect(externalStreamingJobTemplate, "External Streaming Job template should be defined").to
            .not.be.undefined;
        expect(
            externalStreamingJobTemplate.type,
            "External Streaming Job template type should match",
        ).to.equal(ItemType.externalStreamingJob);
        expect(
            externalStreamingJobTemplate.templateScript,
            "External Streaming Job template should contain sp_create_streaming_job",
        ).to.include("sp_create_streaming_job");
    });
    ...
    test("Should have Pre-Deployment Script item template", async function (): Promise<void> {
        ...
        expect(preDeployTemplate, "Pre-Deployment Script template should be defined").to.not.be
            .undefined;
        expect(preDeployTemplate.type, "Pre-Deployment Script template type should match").to.equal(
    ...
    test("Should have Post-Deployment Script item template", async function (): Promise<void> {
        ...
        expect(postDeployTemplate, "Post-Deployment Script template should be defined").to.not.be
            .undefined;
        expect(
            postDeployTemplate.type,
            "Post-Deployment Script template type should match",
        ).to.equal(ItemType.postDeployScript);
    });
    ...
    test("Should have Publish Profile item template", async function (): Promise<void> {
        ...
        expect(publishProfileTemplate, "Publish Profile template should be defined").to.not.be
            .undefined;
        expect(publishProfileTemplate.type, "Publish Profile template type should match").to.equal(
            ItemType.publishProfile,
        );
        expect(
            publishProfileTemplate.templateScript,
            "Publish Profile template should contain Project",
        ).to.include("Project");
    });
    ...
    test("Should have Table-Valued Function item template", async function (): Promise<void> {
        ...
        expect(tvfTemplate.type, "Table-Valued Function template type should match").to.equal(
            ItemType.tableValuedFunction,
        );
        expect(
            tvfTemplate.templateScript,
            "Table-Valued Function template should contain CREATE FUNCTION",
        ).to.include("CREATE FUNCTION");
        expect(
            tvfTemplate.templateScript,
            "Table-Valued Function template should contain RETURNS @returntable TABLE",
        ).to.include("RETURNS @returntable TABLE");
        expect(
            tvfTemplate.templateScript,
            "Table-Valued Function template should contain @@SCHEMA_NAME@@ placeholder",
        ).to.include("@@SCHEMA_NAME@@");
        expect(
            tvfTemplate.templateScript,
            "Table-Valued Function template should contain @@OBJECT_NAME@@ placeholder",
        ).to.include("@@OBJECT_NAME@@");
    });
    ...
    test("Should have Trigger item template", async function (): Promise<void> {
        ...
        expect(triggerTemplate.type, "Trigger template type should match").to.equal(
            ItemType.trigger,
        );
        expect(
            triggerTemplate.friendlyName,
            'Trigger template friendly name should be "Trigger"',
        ).to.equal("Trigger");
        expect(
            triggerTemplate.templateScript,
            "Trigger template should contain CREATE TRIGGER",
        ).to.include("CREATE TRIGGER");
        expect(
            triggerTemplate.templateScript,
            "Trigger template should contain @@SCHEMA_NAME@@ placeholder",
        ).to.include("@@SCHEMA_NAME@@");
        expect(
            triggerTemplate.templateScript,
            "Trigger template should contain @@OBJECT_NAME@@ placeholder",
        ).to.include("@@OBJECT_NAME@@");
    });
    ...
    test("Should have Database Trigger item template", async function (): Promise<void> {
        ...
        expect(dbTriggerTemplate, "Database Trigger template should be defined").to.not.be
            .undefined;
        expect(dbTriggerTemplate.type, "Database Trigger template type should match").to.equal(
            ItemType.databaseTrigger,
        );
        expect(
            dbTriggerTemplate.friendlyName,
            'Database Trigger template friendly name should be "Database Trigger"',
        ).to.equal("Database Trigger");
        expect(
            dbTriggerTemplate.templateScript,
            "Database Trigger template should contain CREATE TRIGGER",
        ).to.include("CREATE TRIGGER");
        expect(
            dbTriggerTemplate.templateScript,
            "Database Trigger template should contain ON DATABASE",
        ).to.include("ON DATABASE");
        expect(
            dbTriggerTemplate.templateScript,
            "Database Trigger template should contain @@OBJECT_NAME@@ placeholder",
        ).to.include("@@OBJECT_NAME@@");
    });
    ...
    test("Should have Sequence item template", async function (): Promise<void> {
        ...
        expect(sequenceTemplate.type, "Sequence template type should match").to.equal(
            ItemType.sequence,
        );
        expect(
            sequenceTemplate.friendlyName,
            'Sequence template friendly name should be "Sequence"',
        ).to.equal("Sequence");
        expect(
            sequenceTemplate.templateScript,
            "Sequence template should contain CREATE SEQUENCE",
        ).to.include("CREATE SEQUENCE");
        expect(
            sequenceTemplate.templateScript,
            "Sequence template should contain @@SCHEMA_NAME@@ placeholder",
        ).to.include("@@SCHEMA_NAME@@");
        expect(
            sequenceTemplate.templateScript,
            "Sequence template should contain @@OBJECT_NAME@@ placeholder",
        ).to.include("@@OBJECT_NAME@@");
        expect(
            sequenceTemplate.templateScript,
            "Sequence template should contain AS BIGINT",
        ).to.include("AS BIGINT");
        expect(
            sequenceTemplate.templateScript,
            "Sequence template should contain START WITH",
